...
class Sync_edoma_house extends CI_Controller {
    ...
    **/
    public function getEdomaHouseToRent()
    {
        ...
        $condition = "comm_id = '".$comm_id."' and edoma_sn > 0 And client_sync = 0 ";
        $edoma_list = $this->it_model->listData( "house_to_rent" , $condition );
	...
	**/
     ...
     * 查詢由edoma平台新增的資料
    **/
    public function getEdomaHouseToRentPhoto()
    {
    /** $_POST = array('comm_id' => 'qazWSXed'
                        ,'edoma_house_to_rent_sn' => 5 );

        dprint('_POST _POST');
        dprint($_POST);
    **/
        $comm_id = tryGetData('comm_id', $_POST, NULL);
        $edoma_house_to_rent_sn = tryGetData('edoma_house_to_rent_sn', $_POST, NULL);
        $condition = "client_sync = 0 aNd edoma_house_to_rent_sn =".$edoma_house_to_rent_sn;
        $edoma_list = $this->it_model->listData( "house_to_rent_photo" , $condition );
